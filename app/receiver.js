...
export async function initReceiver() {
  ...
  if (lib.fsSupported) {
    ...
    if (stored) {
      ...
      if (perm === 'granted') {
        ...
        // permission needs a user gesture → show a reconnect button
  ...
  pickBtn.addEventListener('click', async () => {
    try {
      dirHandle = await lib.pickDirectory();
      lib.hide(reconnectBtn);
      folderName.textContent = t('folder.chosen', getLang(), { name: dirHandle.name });
    } catch (e) {
  ...
  document.addEventListener('langchange', () => {
    ...
    folderName.textContent = dirHandle
      ? t('folder.chosen', getLang(), { name: dirHandle.name })
      : t('folder.empty', getLang());
    updateReceivedCount();
  });
}
...
function setupConnection(conn) {
  ...
  // ensure binary arrives as ArrayBuffer (PeerJS default serialization sets this, but be explicit)
  if (conn.dataChannel) {
...
function writeChunk(buf) {
  ...
  // serialize disk writes so they stay ordered
